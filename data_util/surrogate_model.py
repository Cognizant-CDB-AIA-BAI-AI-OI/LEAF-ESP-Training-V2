...
class SurrogateModel(object):
    ...
    def train_from_data(esp_service, experiment_params, training_data_file, batch_size=1, epochs=1000, verbose=1):
        ...
        # Load the data
        data, labels = SurrogateModel.create_inputs_outputs(experiment_params, training_data_file)
    ...
    def create_inputs_outputs(experiment_params, training_data_csv):
        with open(training_data_csv) as df_file:
            ...
            return SurrogateModel.create_inputs_outputs_df(experiment_params, training_df)

    @staticmethod
    def create_inputs_outputs_df(experiment_params, training_df):
        input_cols = SurrogateModel.gather_input_cols(experiment_params)
        output_cols = SurrogateModel.gather_output_cols(experiment_params)
        #print ("input_cols  ...output_cols .. ",input_cols, output_cols)
        inputs = SurrogateModel._create_df(training_df, input_cols)
        outputs = SurrogateModel._create_df(training_df, output_cols)
    ...
    def gather_input_cols(experiment_params):
        return SurrogateModel._gather_cols(experiment_params['network']['inputs'])

    @staticmethod
    def gather_output_cols(experiment_params):
        return SurrogateModel._gather_cols(experiment_params['network']['outputs'])
...
def train(experiment_params_file, encoded_training_data_csv, output_file, batch_size, epochs, verbose):
    ...
    keras_model = SurrogateModel.train_from_data(esp_service,
                                                    experiment_params,
